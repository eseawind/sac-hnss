...
"t_time<=to_date('"+DateTime.Now+"','yyyy-MM-dd HH24:MI:SS') order by t_time desc) where ROWNUM =1 order by ROWNUM asc";
        ...
        public string  RealValue(string id)
        {
            ...
            string sql = "select t_base_tag.t_tagdesc,t.D_VALUE  from  t_base_tag right join (select T_TAG,D_VALUE from T_INFO_HISTVALUE  where  " +
"T_INFO_HISTVALUE.T_TAG in (select t_tagid from  t_base_tag  where t_tagdesc like '%" + id + "%')) t on t.T_TAG = t_base_tag.t_tagid";
            dt = dl.RunDataTable(sql, out errMsg);
